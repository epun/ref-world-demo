...
- **Landscape**: the geography is **authored** in `src/world/landscape.ts` and is the single
  ...
  **131.7–176.3** [D], inside the ground field's own ±200. Every authored feature keeps at
  ...
  layout moved. **It is 1.32× THAT SIZE on the katamari world** (`MAP_SCALE`; it was 2 from
  2026-09-16, then 1.3 and 1.1 on 2026-09-17 before the same day's *"map is now too small"*)
  — coast **173.8–232.7**, land area 1.74× over — see "one number for the map's size" below.
...
- **One number for the map's size — *(2026-09-16 "make the island twice as big", then
  2026-09-17 "the map is way too big, let's reduce its size by 35%", "I still think this
  island is way too big, let's reduce it by another 15%" and, the same day, "map is now too
  small, let's increase the size of the island by 20%")*.** `MAP_SCALE` in
  `src/world/landscape.ts` has been **2, then 1.3, then 1.1, then 1.32** — read as the
  multiple of the authored DIAMETER, and every step read as LINEAR (2 · 0.65 = 1.3,
  1.3 · 0.85 ≈ 1.1, 1.1 · 1.2 = 1.32).
  ...
  1.1 was the other side of that — a walk that was over before it started — and **1.32 is
  where the island still reads as an island** (a coast in frame at the zoom floor, a forest
  and a range and a lake that do not touch) with a fifth more ground under it than 1.1.
  ...
  **It is no longer an integer, and that is the whole cost of the change.** Every derived
  ...
  counts (253 / 127 / 84 at 1.32, a 5.78-unit coast chord at the widest bearing against the
  same 5.77 at 1.1), the ground field's segments (422 — `320 · 1.32` is 422.4, so at this
  scale the QUAD rounds too, to 1.25118 against the authored 1.25, and a test allows half a
  segment and no more), the three bake resolutions and the physics heightfield. The bakes are
  **rounded to a whole texel count rather than stepped to the next power of two**:
  169² / 338² / 676² over 528 units, which holds every texel within 0.03% of its authored
  size (3.1243 / 1.56213 / 0.78107 against 3.125 / 1.5625 / 0.78125). NPOT is free here — WebGL2, CLAMP, no mipmaps,
  ...
  **The riser run is now a function of the scale** (`riserRun`, `src/world/field.ts`) instead
  ...
  (at 64.5, −133 on the range's apron), **0.5037** at 1.32 (at 80.4, −164.6, the same apron),
  0.5111 at 1.3, 0.4814 at 2. It does not even move monotonically with the map: a twentieth
  more island puts a different hummock under the steepest walk, and 1.32 is a fifth of a unit
  ...
  **What does not** is anything that says HOW BIG a physical thing is: `BEACH_WIDTH` 14,
  ...
  every slope on the map is shallower than it was authored — the steepest measures **0.5037**
  at 1.32 against the field's 0.6 bound (0.4570 at 1.1, 0.4814 at 2, 0.843 authored), and the
  lake island's bank no longer needs the terrain test's explicit exception.
  **A body of water is the one thing a change of scale can break.** A pond's centre scales
  ...
  catches it: **at 1.28–1.35 pond #1 straddles a terrace riser**, with the ground on one side
  a whole tier below its own water line — 0.517 at the 1.3 tried on the way down, **0.478 at
  1.32**, against the 0.6 bound, which is water reading as perched rather than sunk. (At 1.1
  it measured 1.076, the healthiest of the four, and nothing had to move.)
  The fix is where that pond STANDS, never the bound, and never the authored centre either:
  the entry carries an **`islandNudge` of (+5, +5)** (`WaterBody.islandNudge`, applied in
  `applyMapScale` and only on a scaled map), so on the katamari world the pond sits at
  (24.8, −67.6) and **reads 0.8175**. The offset was chosen by measuring the basin-shoulder
  metric over a ±10-unit grid at half-unit steps around the scaled centre and taking the
  NEAREST offset clear of 0.8 — 7.07 units, onto the tread north-east of the riser; nothing
  inside 7 units clears it, so the riser really cannot be dodged by a fraction of a unit. The
  clearances only widen there (39.96 u to the nearest feature edge against the layout's 20,
  114.4 u of dry land to the coast against 12), and the public world still reads (15, −55)
  exactly as it always did. Re-measure the four when the scale moves.
  **Every field that has to cover the land rides the same factor**, and what is held fixed
  across the scale is the RESOLUTION. At 1.32: `FIELD_SIZE` 400 → 528 with `FIELD_SEGMENTS`
  320 → 422, so the quad holds at **1.25118 u** (half a segment off 1.25, the rounding's own
  cost); the three geography bakes keep their texel (region 128² → 169², height 256² → 338²,
  shore 512² → 676², all over 528 u); `GRASS_BASE_SPAN` 360 → 475.2 at the same budget;
  `SCATTER_EXTENT` 160 → 211.2 at the same 6-unit grid step, so the prop count per unit area
  is unchanged; `HEIGHTFIELD_SEGMENTS` 256 → 338 at the same 1.56-u cell; `SPAWN_RADIUS`
  120 → 158.4; `WORLD_MAP_EXTENT` 185 → 244.2; and `GROUND_RADIUS` 1400 → 1848 with the depth
  range that clears it (`cameraDistance` 1800 → 2312, `cameraFar` 3800 → 4624). The sea disc
  has to ride the map for a reason that is not symmetry, and it is a RULE and not a table: at
  the zoom floor on a 390×844 phone the WIDTH binds, so the frame looks ~4× the coast's radius
  up-screen past the island, and the sea has to cover the far CORNER of it — measured
  **980 u** at 1.32 against the 1848-u ring (at 2 the corner was outside a 1400-u ring, which is what made this a
  ...
  island's quarter density; that switch is on `mapScale() !== 1` and still applies at 1.32,
  where the density loss is only a bit over a third — a slightly heavier blade on a slightly
  bigger meadow, and left alone deliberately rather than re-tuned for a scale that has moved
  three times in a day. (A handset draws no blades at all since 2026-09-17.)
  ...
  would have put the far shore out of reach at every zoom, so it rides the scale too (264 at
  1.32).
...
- **The handset's terrain budget — *(2026-09-16, measured at `MAP_SCALE` 2)*.** Four times
  ...
  **Those trades were measured at `MAP_SCALE` 2 and are kept, but the field's one does not
  bind below scale 1.5 — *(2026-09-17)*.** 480 is a CEILING, not a substitute: at 1.32 the
  projection itself cuts 422, so `fieldSegments` takes the `min` and a phone reads the
  projection's 422. A phone must never pay MORE than the projection for a smaller map, which
  is what 480 over 528 units (a 1.1 u quad) would have been. The three texel trades still
  bind and are milder than at 2: 1.03 u a shore texel, 4.13 u a region texel, a 2.06 u
  physics cell. The numbers are kept rather than deleted because they are the measured budget
  for a bigger map and the scale has moved three times in one day; the field's ceiling starts
  binding again above 1.5.
...
- `position.y = baseR · roll` in root-local units, which is `R · roll` in the world: the
  ...
  for a few hours on 2026-09-17; see **The items ARE the mass** below for why the creature
  moved to the middle of its pile. The ramp between is the roll spring's,
...
**The items ARE the mass — no shell** *(2026-09-17)* **[D]**. Three user directions in one
day, each undoing the last, and this is where it landed. In order:

1. *"we should not scale up the characters as they stick to things"* — the `rider`, above: the
   creature keeps its drawn size while the root carries the growth.
2. *"currently there is a bug where the characters are floating in space."* The items were
   seated on the SURFACE of a sphere of radius `bodyR` (`R + itemR × CLUMP_FIT` out from the
   clump's origin) and the creature rode its north pole at `2R` — and nothing drew that sphere,
   so with `GROWTH_K` at 4 and a session's ball ten to twenty metres across the room saw a
   small character hanging in the air over a thin shell of a dozen props. Measured on the real
   build: ground 0.135, root 0.804, `bodyR` 7.125, fifteen items spanning world y −0.279…15.867
   around a centre at 7.93, and the creature's feet at 14.543. A shell was drawn for it
   (`src/creatures/ball.ts`, `BackSide` so it never occluded the creature at its centre).
3. *"the objects that collect around the creatures sit under the creature. I think the creature
   should be at the center, and then it should just be a giant rolling mass."* The pole seat
   went; the creature moved to the pile's centre (`baseR · roll`, which is `clump.group`'s own
   origin — the point every seat is measured from) and the rider took the inverse of the root's
   lean so the mass tumbles and the thing inside it stays upright.
4. **And then the shell itself went:** *"the character should be the object that the items
   stick to."*

So there is no sphere and no mesh for the pile at all. **The character, at its drawn size, is
the body things stick to**, and the visible mass is the items:

- the FIRST item seats on the creature's own radius — `baseR + itemR × CLUMP_FIT`, the same
  bedding that has always made a pile read as one lump rather than a bristle of separate
  objects;
- every one after it PACKS outward along the direction it was struck from: start at the
  creature's surface, and for each seat already taken, if the new item would be inside it,
  slide out until it is only bedded into it by the same `CLUMP_FIT`. Repeat, because sliding
  past one neighbour can bring another into reach (`packSeatDistance`, src/creatures/sticky.ts
  — pure, greedy, one-dimensional, bounded by `PACK_PASSES`).

It is not a packing solver and deliberately not: every item keeps the direction it arrived on,
so the pile is a record of where the creature has been rather than an arrangement. And it runs
**once, on the deciding page** — the packed seat travels on the `stick` event, so no two pages
can pack differently (docs/PLAN.md §7.6's own rule).

**The seats are held in WORLD units** (`Clump.seats()` / `seatOf`), and that is the load-bearing
detail. An event's `ox`/`oy`/`oz` are clump-local — world units over the growth the pile had ON
ARRIVAL — so `add` reads the growth BEFORE inserting the item (which is exactly what the
deciding page did when it computed the offset) and the per-frame write divides the LIVE growth
back out. Hold a packed seat in the clump-local frame instead and it rides the root's uniform
scale: every pickup inflates the packing, a gap opens between every pair of objects, and the
lump becomes the cloud the shell was invented to hide.

**`bodyR` is still the volume's.** `ballDiameter`, the readout, the pickup reach, the resolve
circle, the ground clearance and the physics stand-in are unchanged — that is the GAME's size
and the growth is still one write on the root. The packed pile's outer radius is smaller and the
two drift apart as the pile grows: at fifteen props of drawn radii 0.7–2.9 on a 0.88 u creature
the packed pile reaches ~4.6 u against a `bodyR` of 7.1. The silhouette is the pile; the number
is the game. The stand-in colliders now read the DRAWN item too (`clump.seatOf(key)` and
`item.r`, not `item.r × growth`): a stuck bench used to sweep a circle `growth` times wider
than the bench anybody could see.
...
**A LIVE VIEW OF YOUR OWN BALL, in the corner** *(2026-09-17)* **[D]**. User ask: *"in the top
left hand corner we should show a live view of the character and the objects it collects. the
3d view of the character and the object ball should not scale beyond the radius measurement ui
div in the top left."* The size readout has been in that corner since 2026-09-16; this is the
picture behind it, inside a wavering hairline CIRCLE on the same paper the join code, the
minimap and the leaderboard stand on (TASTE §9a). It is the real creature — its gait, its
emotes, its pile, its roll, whatever is riding it — and not a second model of anything.

**`src/world/portrait.ts`** is one extra pass, after the frame has composed, and every part of
that sentence is a cost decision:

- `renderer.render(root, camera)` takes ANY `Object3D` as its root, so the traversal is the
  creature's own rig and nothing else in the scene is walked, culled or drawn. No layer
  bookkeeping and no visibility toggling of a hundred creature roots a frame — the two other
  ways to draw one subtree, both of which cost the whole scene.
- AFTER `grain.compose`, so the inset takes no ink outline and no grain of its own: the frame's
  two post passes are already on the canvas and this draws into a corner of it. One pass, not
  three.
- no shadows: the stamps live in `shadows.group`, which is not in the subtree.
- **and the subtree carries no lights, because on a katamari world it needs none.** The cel
  chain is self-lit — `applyToon` replaces `opaque_fragment` and writes `outgoingLight` from
  `toonUniforms` (src/world/toon.ts) — so a creature, a ball and a prop are fully shaded with
  no light in the graph. A katamari world on the `ink` style would draw an unlit inset, and
  there is no such world (valiocon is the katamari and valiocon is ghibli). That is the line
  that has to grow a light rig if one is ever added.

**It cannot draw outside the div, and it does not try to.** Two independent bounds:

- a SCISSOR on the rect `src/ui/size.ts` publishes (`BallSizeHandle.rect()`), which is the
  circle's own box — the hard bound, and the reason *"should not scale beyond the … div"* is a
  property of the pass rather than a hope about a camera. GL counts from the bottom, so the
  viewport's y is the canvas height less the rect's bottom edge; three multiplies both by the
  pixel ratio itself, so the numbers are css pixels.
- the FIT: `portraitHalfExtent(bodyR)` is the orthographic half-height, `max(bodyR,
  PORTRAIT_MIN_HALF) × (1 + PORTRAIT_MARGIN)` — never under `bodyR`, so the whole diameter is
  always inside the frame with an eighth of a radius of air. It rides a ζ ≥ 1 spring over
  `MOTION.primaryMs`, because the growth is a STEP (`growth` is instantaneous in the volumes)
  and a view that re-framed on the same frame would be a cut in the one part of the screen a
  person is watching. A creature carrying nothing is framed by `PORTRAIT_MIN_HALF` (2 u, which
  is `CHARACTER_HEIGHT` 3.5 and its stalk, not its footprint).

The camera aims at `portraitAimY(bodyR, roll)` above the ROOT — `bodyR · roll`, which is the
mass's centre and exactly where the rider is, so the creature and the ball are both in the
middle of the frame — and looks along the world's own isometric pair (`ISO_ELEVATION` /
`ISO_AZIMUTH`, now exported from src/world/camera.ts so there is one definition of which way
this world is looked at). FIXED rather than the live camera's: a portrait that swung as the
person orbited would be a second camera to watch, and what is interesting in the corner is the
ball turning under its own roll, which it does because the roll is on the rig this pass draws.

**The PAPER IS A DISC, not the scissor's square.** A scissor is a rectangle, so clearing the
region to the chrome's paper painted a light SQUARE behind a circular mark — a filled panel,
which is the one thing the world brief's mark set forbids (TASTE §4). So the pass clears DEPTH
only and draws the paper itself: a screen-facing `CircleGeometry` scaled to the frustum's
half-extent, so its rim lands exactly on the ring the DOM draws — inside it paper, outside it
the world. One more draw call and ninety-six triangles, and `MeshBasicMaterial` with no
`applyToon`: the cel chain is for things that are in the world, and this is chrome.

**The MARK is the readout's** (src/ui/size.ts): one wavering hairline circle, generated by the
same `wavyRingPoints` + `wavyBorderPath` as the icon beside the number, an earlier sibling of
the row so the number reads over the picture with no second z-index, and its DIAMETER IS THE
ROW'S WIDTH — measured off the live row and eased onto by a ζ ≥ 1 spring, because the row
widens as the number does and a circle that stepped with a digit would be a cut. Bounded
`INSET_MIN_PX` (96) to `INSET_MAX_PX` (132). **No fill in the DOM**: the paper inside the circle
is cleared in GL by the pass, because the DOM is in FRONT of the canvas and a fill up there
would cover the very thing it frames. Nothing else comes with it — no shadow, no radius, no
second fill.

The GATE is the caller: `src/main.ts` installs it inside the same `worldGame === 'katamari' &&
tray?.middle && myDrawerId.length > 0` block as the readout, from inside the same dynamic
import, and `WorldHandles.setPortrait` BUILDS the pass on the first call — so a projection, and
every world without the game, pays nothing for it, not even a camera. The subject is
`ballOwner(myDrawerId)` resolved to its root, so a creature stuck to somebody else's ball
watches that ball, exactly as its readout already reads that ball's size.

Measured on the phone frame at 390×844, dpr 3, swiftshader (`scratch/portrait-smoke.mjs`,
which reads the pixels rather than the code): the inset is 99 px across unladen and 108 px with
a 13.7 m ball — the row's width to the pixel both times — and the drawn content reaches 44.5 px
of a 49.5 px half-width unladen and 48.6 px of 54 px with the ball, covering 96% of the circle
