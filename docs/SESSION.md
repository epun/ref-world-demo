...
| `stick` | `id`, `item`, `kind?`, `variant?`, `scale?`, `r?`, `ox`, `oy`, `oz`, `qx`, `qy`, `qz`, `qw` | creature `id` picked `item` up. `ox`/`oy`/`oz` and the quaternion are its seat **in the carrier's clump-local frame** — every screen seats it identically from those seven floats. `kind`/`variant`/`scale` are mesh hints for a page that never had the placement drawn (all three or none); absent for a creature passenger, which already has a mesh. **`r` is the item's footprint radius** (added 2026-09-17, additively): growth is derived on every page from the radii of what a pile carries, and a viewer read that off the scatter's instance row — which the host hid the moment it took the placement. It fell back to the instance *scale*, ~1 for everything, so a tree that added 1.73 to the host's pile added 1.00 to a phone's and the same ball was two sizes in the same room. Absent reads exactly as it did before; a malformed one is refused with the event, because a zero radius is an item that adds nothing |
...
| `loose` | `item`, `x`, `z`, `scale?` | a rooted prop was knocked out of the ground. **No creature id**: a tree comes out on its own account, and which something hit it is not a thing anybody replays. No kind or variant either — the placement key already spells those out. But **`scale`** does travel (added 2026-09-17, additively): the host has the scatter's instance row and the body it just made, and a viewer has neither, because the placement stopped being drawn the moment this was decided. Without it `showLoose` fell back to 1 and a library model that stood at 2.4 lay there at a third of its size — and `LooseMeshes.show` is idempotent, so that scale survived the `settle` after it and the `stick` after that. Absent reads exactly as before; a malformed one is refused with the event |
